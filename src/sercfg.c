			  ...
			  int parity_mask, int flow_mask)
{
	...
	/*
	 * JDR: Below are the options to add the serial delay function to the initial configuration 
	 * delays are in milliseconds
	 */
	ctrl_editbox(s, "Character delay", '\0', 40,
		HELPCTX(serial_chardelay),
		conf_editbox_handler, I(CONF_serchardelay), I(-1));
	ctrl_editbox(s, "Line delay", '\0', 40,
		HELPCTX(serial_linedelay),
		conf_editbox_handler, I(CONF_serlinedelay), I(-1));
}
