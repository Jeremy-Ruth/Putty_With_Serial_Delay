...
#include <stdlib.h>
#include <limits.h>
#include <time.h>        // JDR: Added to support character and line delay functionality
...
/*
 *  JDR: Typical delay function created using Windows time.h as it seemed to be the
 *  the most compatible with the existing Putty code. This is intended to add a simple feature
 *  not rewrite Putty so a less invasive (but probably not ideal) approach was chosen
 */
static void serial_delay(int delayInMilliseconds)
{
    long long int currStopWatch;
    long long int beginStopWatch;

    beginStopWatch = GetTickCount64();
    while (1) {
        currStopWatch = GetTickCount64();

        /* The additional check for less than zero is for the very unlikely case where a GetTickCounts overflow issue should try to cause an infinite loop */
        if (((currStopWatch - beginStopWatch) >= delayInMilliseconds) || (currStopWatch - beginStopWatch < 0))
            break;
    }
}
...
{
    /* JDR: Added per character and line delays */
    if ((conf_get_int(conf, CONF_serlinedelay) > 0) && (buf[1] == '\0' || buf[1] == "\n")) {
        serial_delay(conf_get_int(conf, CONF_serlinedelay));  
    }else if (conf_get_int(conf, CONF_serchardelay) != 0)
        serial_delay(conf_get_int(conf, CONF_serchardelay));
