...
#include "winsecur.h"
#include "tree234.h"		
#include "serial_add_on.h"			/* access to serial terminal enhancements*/
...
static RECT mainWinPos;							/* JDR: Getting reference of terminal window position for serial delay helper */
...
int WINAPI WinMain(HINSTANCE inst, HINSTANCE prev, LPSTR cmdline, int show)
{
	...
	/* JDR: Only create the serial delay sub-window if the connection type is serial */
	if (conf_get_int(conf, CONF_protocol) == PROT_SERIAL) {
		/* Get current instance reference for serial delay helper window */
		getInstance(&inst);
		getParentWnd(&hwnd);
		regOptWndClss();
		makeOptHwnd();
		/* Position the helper window beside the main window and size it */
		GetWindowRect(hwnd, &mainWinPos);
		SetWindowPos(optHwnd, hwnd, mainWinPos.right, mainWinPos.top, 370, 160, SWP_SHOWWINDOW);
	}

    /*
     * Finally show the window!
	 * JDR: Including the serial sub-window if needed
     */
	if (conf_get_int(conf, CONF_protocol) == PROT_SERIAL)
		ShowWindow(optHwnd, SW_SHOW);
	...
	while (1) {
		...
		if (toplevel_callback_pending() || PeekMessage(&msg, NULL, 0, 0, PM_NOREMOVE)) {
			/*
		...
		while (PeekMessageW(&msg, NULL, 0, 0, PM_REMOVE)) {
			...
			/* If the user focus is the serial option subwindow, */
			/* process the message to the subwindow queue*/
			if (optHwnd != NULL && GetActiveWindow() == optHwnd) 
